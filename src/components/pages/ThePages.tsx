  ...
  return (
    <div className="container text-center">
      ...
      {posts === null ? (
        ...
        <div>
          <div className="generalDimentionOfPosts">{showPosts()}</div>
          <div className="row">
            <div className="offset-2 col-8 menuLanguage">
              <p
                className={
                  underlineMenu.EN ? 'menuLanguageUnderlineTrue' : 'menuLanguageUnderlineFalse'
                }
                onClick={() => {
                  setLanguage('EN');
                  setLanguageMenu(!languageMenu);
                  setUnderlineMenu({ EN: true, RU: false, TC: false });
                }}>
                ENG
              </p>
              <p
                className={
                  underlineMenu.TC ? 'menuLanguageUnderlineTrue' : 'menuLanguageUnderlineFalse'
                }
                onClick={() => {
                  setLanguage('TC');
                  setLanguageMenu(!languageMenu);
                  setUnderlineMenu({ EN: false, RU: false, TC: true });
                }}>
                CHE
              </p>
              <p
                className={
                  underlineMenu.RU ? 'menuLanguageUnderlineTrue' : 'menuLanguageUnderlineFalse'
                }
                onClick={() => {
                  setLanguage('RU');
                  setLanguageMenu(!languageMenu);
                  setUnderlineMenu({ EN: false, RU: true, TC: false });
                }}>
                RUS
              </p>
            </div>
            <div className="col-1 generalSocialNetworks">
              <a href="https://www.linkedin.com/groups/13858939/">
                <BsLinkedin size="19px" />
              </a>
              <a
